...
mf1_observed <- rw_decomp$W %*% rw_decomp$H

mf1_errors <- (mf1_observed - product_ratings)^2
mf1_accuracy <- sqrt(mean(mf1_errors[!is.na(product_ratings)]))

mf1_accuracy
mf1_decomp$n.iteration

mf1_observed <- data.frame(mf1_observed)

product_ratings[sample(length(product_ratings), 30)]

# test_matrix <- round( matrix(runif(30), 10, 3)*10, digits = 0)
# test_sample_ind <- sample(length(test_matrix), ceiling(0.2*length(test_matrix)))
# 
# test_matrix
# test_sample_ind
# test_matrix[test_sample_ind]
