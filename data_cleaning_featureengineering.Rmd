...
rank_1 <- customer_based_recommendations("39263",
                                    cs_2_indexedBy1,
                                    customer_products_bought_wide) %>%
...
rank_2 <- customer_based_recommendations("39263",
                                    customer_similarities1,
                                    customer_products_bought_wide) %>%
  mutate(p_rank = rank(-score)) %>%
  select(product, p_rank)
...
# Item similarity
rank_3 <- product_based_recommendations("39263",
...
# Matrix factorization
rank_4 <- data.frame(score = mf1_predicted["39263",]) %>%
  rownames_to_column("product") %>%
  mutate(p_rank = rank(-score)) %>%
  select(product, p_rank)

# Combination of methods
inner_join(inner_join(inner_join(rank_1, rank_2, by = "product"), rank_3, by = "product"), rank_4, by = "product") %>%
